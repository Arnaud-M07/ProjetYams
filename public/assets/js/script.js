...
function unblockDices(clickEvent) {
  ...
  console.log(
    `La valeur du dé débloqué est ${diceUnclicked}, son index dans keepDices est ${index}`
  );
...
let keepDices1 = [1,2,3,4,5]

// Création d'une fonction somme des différents dès pour remplir le tableau des scores avec un switch
sumOfElements = (keepDices1, combinaison) => {
  switch (combinaison) {
    case "total1":
    let total1 = 0;
      i = 0;
      for (i; i < keepDices1.length; i++) {
        if (keepDices1[i] === valueToSum) {
          total1 += valueToSum;
        }
      }
      return total1;
